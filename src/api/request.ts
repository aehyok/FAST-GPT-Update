...
/* 请求拦截 */
instance.interceptors.request.use(requestStart, (err: any) => Promise.reject(err));
/* 响应拦截 */
instance.interceptors.response.use(responseSuccess, (err: any) => Promise.reject(err));
...
function request(url: string, data: any, config: ConfigType, method: Method): any {
  ...
  return instance
    ...
    .then((res: any) => checkRes(res.data))
    .catch((err: any) => responseError(err));
}
